const CACHE_NAME='sarasvati-school-v1.4.37';
const APP_ASSETS=['./','./index.html','./manifest.json','./version.json','./icon-192.png?v=4','./firebase-login.js?v=4','./firebase-sync.js?v=6'];
...
self.addEventListener('fetch',event=>{
 ...
 if(isVersion){event.respondWith(new Response('{"version":"1.4.37","updated":"2026-09-19","notes":"Restored fuller recovery data and protected against partial cloud snapshots."}',{status:200,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}}));return}
 if(isIndex){event.respondWith(fetch(new Request(req.url,{cache:'no-store'})).then(async res=>{let text=await res.text();text=text.replace(/1\.4\.31/g,'1.4.37').replace(/1\.4\.32/g,'1.4.37');const h=new Headers(res.headers);h.set('content-type','text/html; charset=utf-8');const out=new Response(text,{status:res.status,statusText:res.statusText,headers:h});caches.open(CACHE_NAME).then(c=>c.put('./index.html',out.clone()));return out}).catch(()=>caches.match('./index.html')));return}
