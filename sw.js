const CACHE_NAME = 'sarasvati-school-v1.4.15';
...
self.addEventListener('fetch', event => {
  ...
  if(isVersion){
    event.respondWith(new Response('{"version":"1.4.15","updated":"2026-09-18","notes":"Fixed Studying Certificate print/PDF so only the certificate is printed"}',{status:200,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}}));
    return;
  }
  if(isIndex){
