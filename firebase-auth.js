(function(){
  ...
  async function start(){
    ...
    try{
      ...
      if(typeof window.save==='function'&&!window.save._cloudWrapped){
        const originalSave=window.save;
        const wrapped=function(){originalSave();cloudSave();};
        wrapped._cloudWrapped=true;window.save=wrapped;
      }
      auth.onAuthStateChanged(async u=>{user=u;if(!u){hideApp();msg('Login करें।');return;}showApp();try{await syncInitial();msg('Cloud sync चालू है।',true);}catch(e){msg('Firestore permission error: Firebase Rules check करें।');console.error(e);}});
    }catch(e){msg('Firebase SDK load नहीं हुआ। Internet connection जाँचें।');console.error(e);}
  }
