(function(){
  ...
  let db=null,auth=null,cloudReady=false,saving=false,applying=false;
  ...
  async function cloudSave(){if(!cloudReady||saving||applying)return;saving=true;try{const d=localData();await userRef().set({...d,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});}catch(e){console.warn('Cloud sync save failed',e);}finally{saving=false;}}
  ...
  function wrapSave(){if(typeof window.save!=='function'||window.save._autoCloudWrapped)return;const original=window.save;const wrapped=function(){original();cloudSave();};wrapped._autoCloudWrapped=true;window.save=wrapped;}
