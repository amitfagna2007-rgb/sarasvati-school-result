(function(){
  ...
  function clean(d){
    d=d||{};
    return {students:Array.isArray(d.students)?d.students:[],subjects:d.subjects&&typeof d.subjects==='object'?d.subjects:{},marks:Array.isArray(d.marks)?d.marks:[]};
  }
  function putLocal(d){
    d=clean(d);
  ...
  async function initialSync(){
    ...
    unsub=ref.onSnapshot(s=>{
      ...
      // Firestore can deliver the previous snapshot just after a local edit.
      // Ignore snapshots briefly after a local save so an old cloud copy cannot
      // immediately put the old student information back into the UI.
  ...
  function wrapSave(){
    ...
    const wrapped=function(){lastLocalSaveAt=Date.now();original();recoverySave();setTimeout(()=>cloudSave(),50);};
